...
const AccessControl = () => {
  ...
  const fn_submit = async (e) => {
    ...
    const LogicContractAddress = document.querySelector(
    ...
    try {
      if (pkgDropdown.value == "role") {
        const trx = await accessContract.SetLogicAccount(
        ...
        const trx = await teamBussinessContract.SetLogicContract(
          LogicContractAddress.value
        );
        const reciept = await trx.wait();
        if (reciept) {
        ...
        const trx = await payoutContract.SetLogicContract(
          LogicContractAddress.value
        );
        const reciept = await trx.wait();
        if (reciept) {
  ...
  const fn_payout_submit = async (e) => {
    e.preventDefault();
    ...
    if (LogicContractAddress.value == "") {
      alert("Enter payout contract address");
      return;
    }
    try {
      const trx = await accessContract.SetPayoutAccount(
        LogicContractAddress.value
      );
      const reciept = await trx.wait();
      if (reciept) {
  ...
  const fn_team_submit = async (e) => {
    ...
    if (LogicContractAddress.value == "") {
      alert("Enter team contract address");
      return;
    }
    try {
      const trx = await accessContract.SetTeamBussinessAccount(
        LogicContractAddress.value
      );
      const reciept = await trx.wait();
      if (reciept) {
...
<hr/>
      <div className="card">
        ...
        <div className="card-body">
          <div className="row">
            <div className="form-group">
              <label>Payout Contract Address</label>
              <input
                type="text"
                className="form-control"
...
<hr/>
       <div className="card">
        ...
        <div className="card-body">
          <div className="row">
            <div className="form-group">
              <label>Team Contract Address</label>
              <input
                type="text"
                className="form-control"
                id="TeamContractAddress"
              />
            </div>
          </div>
        </div>
